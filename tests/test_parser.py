...
def test_end_to_end_simple(fixture_dir):
    ...
    input_file = pathlib.Path(fixture_dir) / "simple.md"
    with tempfile.TemporaryDirectory() as tmpdir:
        outputs = parse_markdown(input_file, pathlib.Path(tmpdir))
...
def test_end_to_end_with_frontmatter(fixture_dir):
    ...
    input_file = pathlib.Path(fixture_dir) / "with_frontmatter.md"
    with tempfile.TemporaryDirectory() as tmpdir:
        outputs = parse_markdown(input_file, pathlib.Path(tmpdir))
        assert len(outputs) == 1
        assert outputs[0].name.endswith(".pdf")
...
def test_end_to_end_with_inline(fixture_dir):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        input_file = pathlib.Path(tmpdir) / "with_inline.md"
        # Copy inline target to tmpdir so it's relative to the markdown file
        inline_target = str(pathlib.Path(tmpdir) / "inline_target.md")
        with pathlib.Path(inline_target).open("w", encoding="utf-8") as f:
            f.write("Inlined: {{name}}")

        with pathlib.Path(input_file).open("w", encoding="utf-8") as f:
            f.write("""# Document
...
def test_keep_intermediate_false(fixture_dir):
    ...
    input_file = pathlib.Path(fixture_dir) / "simple.md"
    with tempfile.TemporaryDirectory() as tmpdir:
        parse_markdown(input_file, pathlib.Path(tmpdir), keep_intermediate=False)
        # Should only have PDF files
        all_files = [f.name for f in pathlib.Path(tmpdir).iterdir()]
...
def test_keep_intermediate_true(fixture_dir):
    ...
    input_file = pathlib.Path(fixture_dir) / "simple.md"
    with tempfile.TemporaryDirectory() as tmpdir:
        parse_markdown(input_file, pathlib.Path(tmpdir), keep_intermediate=True)
        all_files = [f.name for f in pathlib.Path(tmpdir).iterdir()]
...
def test_max_iterations_exceeded_self_referencing():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        self_ref_file = str(pathlib.Path(tmpdir) / "self_ref.md")
        with pathlib.Path(self_ref_file).open("w", encoding="utf-8") as f:
        ...
        with pytest.raises(
            ...
            process_directives_iteratively(content, pathlib.Path(tmpdir), False)
...
def test_multilingual_build_logs_translation_warnings(caplog):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        input_file = pathlib.Path(tmpdir) / "test.md"
        with pathlib.Path(input_file).open("w", encoding="utf-8") as f:
            f.write("""---
multilingual: true
base_language: en
...
""")
        ...
        translations_dir = str(pathlib.Path(tmpdir) / "test")
        pathlib.Path(translations_dir).mkdir(exist_ok=True, parents=True)
        ...
        de_po = str(pathlib.Path(translations_dir) / "de.po")
        with pathlib.Path(de_po).open("w", encoding="utf-8") as f:
...
""")
        ...
        with patch("subprocess.run"):
            # Build multilingual PDF
            parse_markdown(input_file, pathlib.Path(tmpdir))
...
def test_dpi_frontmatter_parameter(fixture_dir):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        input_file = pathlib.Path(tmpdir) / "test_dpi.md"
        with pathlib.Path(input_file).open("w", encoding="utf-8") as f:
            f.write("""---
dpi: 300
---
...
def test_dpi_frontmatter_with_image():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        img_path = str(pathlib.Path(tmpdir) / "test_image.png")
        img.save(img_path, dpi=(600, 600))

        # Create CSS file with image sizing
        css_path = str(pathlib.Path(tmpdir) / "style.css")
        with pathlib.Path(css_path).open("w", encoding="utf-8") as f:
...
""")
        ...
        outputs_300 = parse_markdown(input_file, pathlib.Path(tmpdir))
        assert len(outputs_300) == 1

        # Generate PDF without DPI
        input_file_no_dpi = pathlib.Path(tmpdir) / "test_no_dpi.md"
...
""")
        ...
        outputs_no_dpi = parse_markdown(input_file_no_dpi, pathlib.Path(tmpdir))
...
def test_dpi_validation_warns_on_low_dpi_images(caplog):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        img_path = str(pathlib.Path(tmpdir) / "lowres.png")
        img.save(img_path)
