...
End"""
    result = process_inlines(content, pathlib.Path())
...
<!-- inline:"tests/fixtures/nonexistent.md" -->"""
    with pytest.raises(FileNotFoundError):
        process_inlines(content, pathlib.Path())
...
<!-- inline:"tests/fixtures/simple.py" -->"""
    result = process_inlines(content, pathlib.Path(), allow_python=True)
...
<!-- inline:"tests/fixtures/simple.py" -->"""
    with pytest.raises(ValueError, match="Python file execution not allowed"):
        process_inlines(content, pathlib.Path(), allow_python=False)
...
<!-- inline:"tests/fixtures/error.py" -->"""
    with pytest.raises(ValueError, match="Python execution error"):
        process_inlines(content, pathlib.Path(), allow_python=True)
